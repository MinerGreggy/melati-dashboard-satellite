...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'melati-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('melati').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('melati')[service]);
